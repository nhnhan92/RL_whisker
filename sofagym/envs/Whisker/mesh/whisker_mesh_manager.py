    ...
    def chamber(no_chamber,chamber_bot_radius,cone_angle,chamber_height,mesh_size):
        ...
        if no_chamber == 2:
            ...
            for i in range(no_chamber):
                ...
                gmsh.model.occ.synchronize()
                chamber = gmsh.model.occ.revolve([(1, l2),(1, l3),(1, l4)], rot_axis[i][0], rot_axis[i][1], 
                                                rot_axis[i][2], rot_axis[i][3], rot_axis[i][4], rot_axis[i][5], chamber_rel_angle)
                gmsh.model.occ.synchronize()
                l5 = gmsh.model.occ.addLine(p1, p4+1, tag=-1)
                curve1 = gmsh.model.occ.addCurveLoop([l1, l2, l3, l4], -1)
                chamber_plane1 = gmsh.model.occ.addPlaneSurface([curve1], tag=-1)
                # if i == 0:
                curve2 = gmsh.model.occ.addCurveLoop([4+9*i, 8+9*i, 7+9*i, 9+9*i], -1)
                chamber_plane2 = gmsh.model.occ.addPlaneSurface([curve2], tag=-1)
                gmsh.model.occ.synchronize()
        else:
            rot_axis = [[0, 0, 0, 0, 0, 1]]
...
class whisker_body_mesh():
    ...
    def main_body(body_bot_radius,
        ...
        ### Chambers
        chamber = chamber_mesh_full.chamber(no_chamber = no_chamber,
...
def main():
    ...
    # chamber_mesh.chamber(no_chamber = 1,
    ...
    gmsh.model.occ.mesh.setSize(gmsh.model.occ.getEntities(-1), 4)
    gmsh.model.occ.synchronize()
